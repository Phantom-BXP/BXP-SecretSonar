...
class TwitchValidator(GenericHttpValidator):
    ...
    async def validate(self, candidate: Candidate) -> Validated:
        ...
        try:
            client = self._get_client("twitch")
            async with client:
                resp = await client.get("https://api.twitch.tv/helix/users", headers={"Authorization": f"Bearer {secret}", "Client-Id": "none"})

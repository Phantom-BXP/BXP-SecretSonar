...
class DiscordValidator(GenericHttpValidator):
    ...
    async def validate(self, candidate: Candidate) -> Validated:
        ...
        try:
            client = self._get_client("discord")
            async with client:
                resp = await client.get("https://discord.com/api/v10/users/@me", headers={"Authorization": f"Bot {secret}"})
                if resp.status_code == 200:
                    ...
                    validated.candidate.evidence.metadata["discord_username"] = data.get("username", "")

...
class MailgunValidator(GenericHttpValidator):
    ...
    async def validate(self, candidate: Candidate) -> Validated:
        ...
        try:
            client = self._get_client("mailgun")
            async with client:
                resp = await client.get("https://api.mailgun.net/v4/domains", auth=("api", secret))
                if resp.status_code == 200:
                    ...
                    validated.candidate.evidence.metadata["mailgun_domains"] = len(data.get("items", []))

...
class GitLabValidator(GenericHttpValidator):
    ...
    async def validate(self, candidate: Candidate) -> Validated:
        ...
        try:
            client = self._get_client("gitlab")
            async with client:
                resp = await client.get("https://gitlab.com/api/v4/user", headers={"PRIVATE-TOKEN": secret})
                if resp.status_code == 200:
                    ...
                    validated.candidate.evidence.metadata["gitlab_user"] = data.get("username", "")

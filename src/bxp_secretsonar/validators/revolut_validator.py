...
class RevolutValidator(GenericHttpValidator):
    ...
    async def validate(self, candidate: Candidate) -> Validated:
        ...
        try:
            client = self._get_client("revolut")
            async with client:
                resp = await client.get("https://api.revolut.com/api/1.0/accounts", headers={"Authorization": f"Bearer {secret}"})
                if resp.status_code == 200:
                    ...
                    validated.candidate.evidence.metadata["revolut_accounts"] = len(data)
